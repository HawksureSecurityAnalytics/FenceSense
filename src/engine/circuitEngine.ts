...
export function traceCircuit(
  ...
  const check = (id: string) => {
    if (faultIds.has(id) && faultStep === null) faultStep = path.length - 1;
  };

  path.push({ kind: 'energizer_out', id: 'enrg_out', label: 'Energizer + (Live Out)' });

  // Serpentine: HT strands go left→right then right→left alternating
  htS.forEach((si, k) => {
    ...
    if (k < htS.length - 1) {
      ...
      path.push({
        ...
        bridgeId: br?.id,
        strandIndex: si,
        label: `HT Bridge ${si + 1}→${htS[k + 1] + 1} at ${side} end`,
      });
  ...
  // Earth strands serpentine in reverse direction
  eS.forEach((si, k) => {
    ...
    if (k < eS.length - 1) {
      ...
      path.push({
        ...
        bridgeId: br?.id,
        strandIndex: si,
        label: `Earth Bridge ${si + 1}→${eS[k + 1] + 1} at ${side} end`,
      });
      if (!br && faultStep === null) faultStep = path.length - 1;
      else if (br) check(br.id);
    }
